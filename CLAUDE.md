...
- **Keep your phase code in `analysis/screening.py`** (created by `fsp init`), filled **one part at a time** — run a part, read its output, decide, document, gate, *then* the next; never batch-write the whole run (§1, §3.1). Split a phase into its own file under `analysis/` if it grows. Run outputs (notebook, ledger, folds, manifest) land under `runs/<run-id>/`.
